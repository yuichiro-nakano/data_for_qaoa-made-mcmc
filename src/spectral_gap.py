...
def main():
    ...
    gap_data = np.zeros((n_beta, 4, n_instance))
    ...
    for beta in beta_list:
        ...
        for k in range(n_instance):
            ...
            if init_para == None:
                para = [rng.uniform(0, 2*np.pi) for i in range(2*n_layers)]
            else:
                para = init_para

            opt = scipy.optimize.minimize(cost, init_para, method=method, options=options)

            # sampling from QAOA distribution
            qaoa_opt_data_idx = qaoa.sampling_QAOA(qaoa_ansatz, opt.x, n_train+n_test) # optimize parameter
            qaoa_fix_data_idx = qaoa.sampling_QAOA(qaoa_ansatz, init_para, n_train+n_test) # fixed angle
...
if __name__ == '__main__':
    ...
    method = "BFGS"
    options = {"disp": False, "maxiter": 200, "gtol": 1e-6}
